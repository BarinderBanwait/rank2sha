...
/-!
# Eisenstein–Kronecker interface: `EKPackage` (task R1γ)

The archimedean side of `def:deltaE`. `EKPackage` bundles the divisor `D_E` of `eq:DEdef`,
the six-function package `𝓡_E` of `eq:jetpackage`, a multiplicative valuation at each
rational prime, and the support function `supp` of `hyp:sinnott`. On top of that data,
`δ_E(c)` is a **definition**, not an assumed datum:

    EKPackage.Fc     P c t = ∑ i, c i • P.r i t
    EKPackage.deltaE P c   = ∏ t ∈ P.D, P.Fc c t

The structure this file replaces carried `δ_E` as opaque data (`ClassicalInputs.deltaE : ℚ`,
`KatzData.deltaE_local : ℚ`) together with an assumed link to a nonvanishing predicate. Here
the product is written out, so `def:deltaE` is discharged by unfolding and the only remaining
data are the objects the paper itself constructs: `D_E`, `𝓡_E`, `v_𝔭`, `supp`.
...
Every inequality of `hyp:sinnott` and `thm:reduction` therefore reads reversed in the Lean.
...
`L` is the paper's `Q̄`. That `δ_E(c) ∈ K` is `lem:orbit`(1), which the paper proves only after
granting the equivariance `r_{a,b}(σ t) = σ(r_{a,b}(t))`, an input it uses rather than proves.
`thm:reduction` does not use `lem:orbit`, and `δ_E(c)` is not proved rational
(`deltaE.tex`, after `thm:reduction`), so the divisibility condition `𝔭 ∤ δ_E(c)` is not
...
Paper labels quoted below: `eq:DEdef`, `eq:jetpackage`, `def:deltaE`, `lem:orbit`,
`hyp:sinnott`, `thm:reduction`.
-/

namespace FinShaRank2

/-- The six slots of `eq:jetpackage`, as the pairs `(a, b)` with `a ≥ 0`, `b ≥ 1`, `a + b ≤ 3`,
...
PAPER: `eq:jetpackage`. STATUS: definition. -/
...
/-- `jetIndex` enumerates the index set of `eq:jetpackage` — the pairs `(a, b)` with `b ≥ 1`
...
/-- **Eisenstein–Kronecker package for a CM elliptic curve `E/ℚ`** (paper §`ssec:deltaEdef`).

The data of `def:deltaE` other than the coefficient vector: the divisor `D_E` of `eq:DEdef`,
the six functions `r_{a,b}` of `eq:jetpackage`, the valuation `v_𝔭` at each rational prime, and
the support function of `hyp:sinnott`. The invariant `δ_E(c)` is then the def
...
structure EKPackage where
  ...
  coefficient vectors `c` of `def:deltaE` are elements of `K⁶`.
  SOURCE: the paper's own definition.
  PAPER:  `def:deltaE` (`c = (c_{a,b}) ∈ K⁶`). STATUS: data. -/
  K : Type
  ...
  PAPER:  `eq:jetpackage` (`r_{a,b} : D_E → Q̄`). STATUS: data. -/
  L : Type
  ...
  SOURCE: the paper's own definition. PAPER: §`ssec:notation` (`v_𝔭` on `Q̄^×`).
  STATUS: data. -/
  Γ : Type
  ...
  SOURCE: the paper's own definition. PAPER: `eq:DEdef`. STATUS: data. -/
  ι : Type
  ...
  PAPER:  `eq:DEdef`. STATUS: classical. -/
  ...
  PAPER:  `eq:jetpackage`. STATUS: data. -/
  r : Fin 6 → ι → L
  /-- The valuation `v_𝔭` on `L`, indexed by the rational prime `p`; `𝔭` is the prime of `Q̄`
  determined by the embedding `ι_p` fixed in §`ssec:notation`. **Multiplicative**: `v_𝔭(x) = 0`
  in the paper is `P.v p x = 1` here, and `v_𝔭(x) ≥ 0` is `P.v p x ≤ 1`. See the module
  docstring.
  SOURCE: classical (valuation theory).
  PAPER:  §`ssec:notation`; used in `hyp:sinnott` and `thm:reduction`. STATUS: data. -/
  v : ℕ → Valuation L Γ
  /-- `supp(c)`: the finite set of rational primes lying below a prime of `Q̄` at which some
  entry of `c` is not a unit. Left opaque. `thm:reduction` uses it only through the exclusion
  ...
  SOURCE: the paper's own definition.
  PAPER:  `hyp:sinnott` (the definition of `supp(c)`). STATUS: data. -/
...
PAPER: `def:deltaE`. STATUS: definition. -/
def Fc (t : P.ι) : P.L := ∑ i, c i • P.r i t

/-- `δ_E(c) := Res(F_c, D_E) = ∏_{t ∈ D_E} F_c(t)`, the candidate invariant of `def:deltaE`.

It lands in `P.L`, the paper's `Q̄`. That it lies in `K` is `lem:orbit`(1), which needs the
equivariance input the paper does not prove; `thm:reduction` does not use it.
PAPER: `def:deltaE`. STATUS: definition. -/
...
/-- `δ_E(c) ≠ 0` exactly when `F_c` vanishes at no point of `D_E`. Immediate from `def:deltaE`
once `δ_E(c)` is the product rather than assumed data.
PAPER: the sentence following `def:deltaE`. STATUS: definition (consequence of unfolding). -/
