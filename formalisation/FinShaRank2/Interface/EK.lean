...
The formal counterpart of the Bannai–Kobayashi jet: the divisor `D_E` of (10), six
of the class functions `r_{a,b}` of Definition 5.1, and a multiplicative valuation at
each rational prime. Only the objects the paper itself constructs are assumed.
...
## The index set is a truncation

Definition 5.1 defines `r_{a,b}` for every `a ≥ 0` and `b ≥ 1`. `EKPackage.r` carries
the six slots with `a + b ≤ 3`, which `jetIndex` enumerates. That truncation is the
formalisation's own; the paper states no six-element index set.

No declaration outside this file and `Toy/EK.lean` reads a section value: `EKPackage`
is part of the assumption surface, and the criterion the class functions feed —
Theorem B (Theorem 6.7) — has no Lean counterpart.

Paper statements quoted below: (10), Definition 5.1, (12).
-/

namespace FinShaRank2

/-- The six slots of `EKPackage.r`, as the pairs `(a, b)` with `a ≥ 0`, `b ≥ 1` and
`a + b ≤ 3`, graded by `a + b - 1`.

`EKPackage.r` is indexed by `Fin 6` rather than by pairs, so this def records which slot is
which. `jetIndex_image` checks that the enumeration is exactly that set of pairs.
PAPER: Definition 5.1 (`def:classsums`) (the class functions `r_{a,b}`, there at every
index `a ≥ 0`, `b ≥ 1`). STATUS: definition. -/
...
/-- `jetIndex` enumerates the pairs `(a, b)` with `b ≥ 1` and `a + b ≤ 3`, without
repetition. -/
...
The data of the Bannai–Kobayashi jet: the divisor `D_E` of (10), six of the class
functions `r_{a,b}` of Definition 5.1, and the valuation `v_𝔭` at each rational prime.
...
structure EKPackage where
  ...
  section values lie in a `K`-algebra.
  ...
  PAPER:  Definition 5.1 (`def:classsums`) (`r_{a,b} : D_E → Q̄`). STATUS: data. -/
  L : Type
  ...
  /-- Six class functions `r_{a,b} : D_E → Q̄`, indexed by `Fin 6` through `jetIndex`:
  the grade-`≤ 2` jet of the reduced theta function evaluated along the `𝔣`-division
  values, `r_{a,b}([g]) = ε(g)^{-(a+b)} e^*_{a,b}(t_g, 0) / A(Γ)^a`.
  ...
  PAPER:  Definition 5.1 (`def:classsums`), (12) (`eq:classsums`). STATUS: data. -/
  r : Fin 6 → ι → L
  ...
  PAPER:  §2.1 (`ssec:notation`), (12) (`eq:classsums`). STATUS: data. -/
  v : ℕ → Valuation L Γ
