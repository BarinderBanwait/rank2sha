...
rank-two CM elliptic curves* (paper §4).

`KatzData p Lp` packages the algebraic phase of the programme:

* the comparison `L_p(E,T) = c_p · u(T) · L^{Katz}_𝔭(T)` (Lemma 3.7);
* the grading congruence reducing `v_p(c₂(L^K)) = 0` to nonvanishing of the
  mod-𝔭 **criterion class** built from grade-two moments (Proposition 4.1,
  Proposition 4.2);
...
carries the package and the vector, and Theorem C (Theorem 4.12) routes the resultant
...
Hypothesis 4.10, Conjecture 4.11 — is kept OUT of `KatzData` and lives only in
...
`SinnottHyp` below states Hypothesis 4.10's integrality and nonvanishing clauses
...
Paper statements quoted below: Lemma 3.7, Proposition 4.1, Proposition 4.2,
Lemma 4.3, Definition 4.8, Hypothesis 4.10, Conjecture 4.11, Theorem C.
-/
...
/-- **Katz data at a split prime `p`** (paper §4), relative to the MTT
p-adic L-function `Lp : Λ p`.

The coefficient ring `W` is the completion of the maximal unramified extension
of `ℤ_[p]` (`main.tex` §4 preamble): a local domain, `ℤ_[p]`-algebra,
...
structure KatzData (p : ℕ) [Fact p.Prime] (Lp : Λ p) where
  ...
  of `ℤ_[p]` (`main.tex` §4: "`W` denotes the completion of the
  ...
  PAPER: §4 (§4) preamble. STATUS: data. -/
  W : Type
  ...
  PAPER: §4 (§4) preamble. STATUS: classical. -/
  ...
  (`L^K` of Proposition 4.1; `L^{Katz}_𝔭` of Lemma 3.7).
  ...
  PAPER: Lemma 3.7 (`lem:comparison`), Proposition 4.1 (`prop:jetformula`). STATUS: data. -/
  LKatz : PowerSeries W
  /-- **Comparison** (Lemma 3.7): after base change to `W`, the MTT
  ...
  this bakes in the conclusion `v_p(c_p) = 0` for `p ∉ S` (Lemma 3.7,
  ...
  PAPER: Lemma 3.7 (`lem:comparison`) (`L_p(E,T) = c_p · u(T) · L^{Katz}_𝔭(T)`).
  ...
  moment sum, `M₂(𝔭) = p² · m2core` (Proposition 4.2(2) divisibility
  `v_p(M₂(𝔭)) ≥ 2`). Its residue mod 𝔭 is the criterion class `𝔠(𝔭)`
  (`criterionClass`).
  SOURCE: Proposition 4.2 (`prop:grading`)(2) (`M₂(𝔭) = p² · (𝔠(𝔭)-lift)`); de Shalit II §4 for
  the local moment terms.
  PAPER: Proposition 4.2 (`prop:grading`)(2). STATUS: data. -/
  m2core : W
  /-- **Grading congruence** (Proposition 4.1 + Proposition 4.2(1)): for a unit
  ...
  **Honest descope.** Proposition 4.1 expresses `c₂(L^K) = ½∫m² dμ_ψ` as a
  second moment and Proposition 4.2(1) gives `2 log_p(1+p)² c₂(L^K) ≡ M₂(𝔭)
  (mod p³ W)`; the Eisenstein–Kronecker bookkeeping identifying `M₂(𝔭)`/`m2core`
  "to the last constant" (Hypothesis 4.10(i)) is **not** carried in Lean. This
  ...
  PAPER: Proposition 4.1 (`prop:jetformula`), Proposition 4.2 (`prop:grading`)(1).
  ...
  data): the trace, over the 𝔭-torsion translates of §4.3, of the
  reductions of the fixed grade-two combination of the sections `𝓡_E`. Its
  equality with `criterionClass` is Hypothesis 4.10(i) (a `SinnottHyp` field, not
  asserted here); its nonvanishing under pointwise `𝔭`-unitness of `F_c` on
  `D_E` is Hypothesis 4.10(ii).
  ...
  PAPER: Hypothesis 4.10 (`hyp:sinnott`), §4.3 (§4.3). STATUS: opaque data. -/
...
of Proposition 4.2, as the residue of `m2core`.

This is a **def, not a field**: it is the bridge
that `SinnottHyp.presentation` (Hypothesis 4.10(i)) equates with the constructed
`traceClass`, and the object the grading-valuation kernel (`SelmerData.π_surj`) links to
`IsUnit (coeff 2 LKatz)`.
PAPER: Proposition 4.2 (`prop:grading`) (the criterion class `𝔠(𝔭)`). -/
...
Provided so that the unit-transfer step of Theorem C (Theorem 4.12)
...
/-- **Sinnott trace property for second jets** (Hypothesis 4.10) — a
`Prop`-valued structure used ONLY in **hypothesis position** of Theorem C (Theorem
4.12). It is referenced by no instantiable structure, so
...
package `P` and the coefficient vector `c` of Hypothesis 4.10(i), so that the
...
structure SinnottHyp (p : ℕ) [Fact p.Prime] (Lp : Λ p) (Kd : KatzData p Lp)
  ...
  /-- **Hypothesis 4.10(i), integrality clause.** The values `F_c(t)`, `t ∈ D_E`,
  ...
  PAPER: Hypothesis 4.10 (`hyp:sinnott`)(i). -/
  integral : ∀ t ∈ P.D, P.v p (P.Fc c t) ≤ 1
  /-- **Hypothesis 4.10(i), presentation clause.** The criterion class `𝔠(𝔭)` of
  Proposition 4.2 admits the presentation constructed in §4.3: it is
  ...
  /-- **Hypothesis 4.10(ii) (Nonvanishing).** If `F_c` is a `𝔭`-unit at every point
  ...
  PAPER: Hypothesis 4.10 (`hyp:sinnott`)(ii). -/
