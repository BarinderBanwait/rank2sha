...
/-- **Katz data at a split prime `p`** (paper §7), relative to the MTT
p-adic L-function `Lp : Λ p`.

The coefficient ring `W` is the completion of the maximal unramified extension
of `ℤ_[p]` (`main.tex` §2.8): a local domain, `ℤ_[p]`-algebra,
...
structure KatzData (p : ℕ) [Fact p.Prime] (Lp : Λ p) where
  ...
  of `ℤ_[p]` (`main.tex` §2.8: "`W` denotes the completion of the
  ...
  PAPER: §2.8 (`ssec:katz`). STATUS: data. -/
  W : Type
  ...
  PAPER: §2.8 (`ssec:katz`). STATUS: classical. -/
