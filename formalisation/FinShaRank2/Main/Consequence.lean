...
/-!
# Theorem A (Theorem 3.8) — the arithmetic consequence of a unit second jet

The paper's statement, which `prop_consequence` below renders:

> **Theorem A (Theorem 3.8).** *Let `p ≥ 5` be a split prime of good
...
/-- **Theorem A (Theorem 3.8)** at a split prime `p ∉ H.S` whose normalised second jet
