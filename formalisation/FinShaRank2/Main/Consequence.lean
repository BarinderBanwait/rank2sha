...
/-!
# Theorem A (Theorem 4.9) — the arithmetic consequence of a unit second jet

The paper's statement, which `prop_consequence` below renders:

> **Theorem A (Theorem 4.9).** *Let `p ≥ 5` be a split prime of good
...
proof, which records that nothing in it assumes Conjecture 4.7.
...
## How non-anomality reaches Proposition 4.3
...
Paper statements rendered here: Theorem A, Lemma 2.3, (10),
Proposition 4.3.
-/
...
/-- **Non-anomality at a split prime `p ∉ S`, in the form Proposition 4.3
consumes.** `ClassicalInputs.notAnomalous` is the `S_an` clause of (10),
...
PAPER: (10) (`eq:Sexc`) (`S_an`), Definition 2.2 (`def:anomalous`), Proposition 4.3
(`prop:normalisation`). -/
...
/-- **Theorem A (Theorem 4.9)** at a split prime `p ∉ H.S` whose normalised second jet
...
Assembly route for `prop_consequence`: Proposition 4.3 via
`Kernel.Normalization.isPUnit_c2tilde_iff`, fed non-anomality by
`ClassicalInputs.isPUnit_one_sub_alphaInv`, turns `hc2` into
`IsUnit (coeff 2 Lp)`; Lemma 4.1 (`c0_eq_zero`/`c1_eq_zero`) gives `c₀ = c₁ = 0`;
...
theorem prop_consequence (H : ClassicalInputs) {p : ℕ} [Fact p.Prime]
  ...
  -- **step 1** Proposition 4.3: the normalised jet is a unit iff `c₂` is.
  --   Non-anomality comes from `H.notAnomalous`, the `S_an` clause of (10).
  ...
  -- **steps 2 and 3** Lemma 4.1 (`c0_eq_zero`/`c1_eq_zero`), applied to the bundle's `AnalyticData`.
