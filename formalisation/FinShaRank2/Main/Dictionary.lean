...
/-!
# Proposition 4.2 — the height/analytic dictionary

The paper's statement, which `prop_dictionary` below renders:

> **Proposition (Dictionary, Proposition 4.2).** *Let `p ∉ S` be a non-anomalous
...
through Theorem A (Theorem 3.8) and Proposition 3.3. Against the frozen
...
* `PrimeData.c2norm_tie` identifies `(…).height.c2norm` with the normalised jet
  `(…).c2tilde` **definitionally** (`ClassicalInputs` tie-equation), so (1) is a statement about
...
* `PrimeData.shaOrd_tie : IsPUnit shaOrd ↔ Subsingleton ShaDual` is the `KatzData` and
`ClassicalInputs`
...
/-- **Proposition 4.2** at a split prime `p ∉ H.S`.
...
`HeightData.spr_nondeg` (nondegeneracy, regulator, Ш) so that `prop_dictionary`'s proof is a
...
theorem prop_dictionary (H : ClassicalInputs) {p : ℕ} [Fact p.Prime]
  ...
  -- leading-term theorem) and Step 3 (`PrimeData.shaOrd_tie`, the `KatzData` and `ClassicalInputs`
