...
/-!
# `lem:c0c1` — vanishing of the first two Taylor coefficients (task T30)

Frozen headline signature (T15 statement freeze; `TASK_BOARD.md` §2 conv. 7).
The proofs are supplied by task T30.

> **Lemma (`lem:c0c1`).** *Assume `L(E,1) = 0` and `w(E) = +1`. Then for every good
...
* `L(E,1) = 0` is the certificate field `msymb_zero : modularSymbol0 = 0`, which
  ...
  character) to give `c₀(p) = 0` — this is convention 3 (data-equations over bare
  Props) in action: the vanishing is *derived*, not assumed;
...
Downstream (`prop:consequence`, T31) they are applied to
`(H.dataAt p hp hsplit hpS).analytic`.

Paper labels rendered here: `lem:c0c1`.
-/
...
/-- **`lem:c0c1`, first half — `c₀(p) = 0`.**

> *`c₀(p) = L_p(E,0) = (1 − α_p⁻¹)² · L(E,1)/Ω_E = 0`.*

Derived (task T30) from the MTT interpolation data-equation `A.interp` together
...
Derived (task T30) from the field `A.funct_eq` — the functional equation with the
root number `w(E) = +1` already folded in — via the kernel lemma
`FinShaRank2.coeff_one_Lp_eq_zero` (T20, `Kernel/FunctionalEquation.lean`), which
