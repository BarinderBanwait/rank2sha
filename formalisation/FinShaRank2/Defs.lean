...
on top of these definitions, and the kernel tasks T20–T27 reason about them.
Everything here is therefore a **standalone** definition of explicit arguments.

Contents:

* `Λ p` — the Iwasawa algebra `ℤ_[p]⟦X⟧` (mathlib `PowerSeries ℤ_[p]`).
* `σ` — the functional-equation substitution representing `(1 + T)⁻¹ − 1`,
  with the coefficient facts and the `HasSubst` witness that task T20 (the
  functional-equation coefficient lemma, paper `lem:c0c1`) consumes.
* `IsPUnit` — the p-adic unit predicate `‖x‖ = 1` on `ℚ_[p]`.
* `MuZero`, `lambdaAn` — the Iwasawa μ = 0 predicate and analytic λ-invariant of
  a power series (paper `prop:consequence`, Step 2).
* `c2tilde` — the normalised second jet `def:c2tilde`.
...
* `σ` is fixed at coefficient ring `ℤ_[p]` (its type is `Λ p`). A task needing
  ...
  ring-generic half of T20) should replicate the one-line `PowerSeries.mk`; the
...
with sign `w(E) = +1`; this is the input to `lem:c0c1` (see `oneAddX_mul_σ` and
...
Task T20 uses this to turn the functional equation into `c₁ = −c₁`, forcing
`c₁ = 0` (`lem:c0c1`). -/
...
feeds the functional-equation hypothesis of `AnalyticData` (T11) and the kernel
lemma T20. Tool: `PowerSeries.HasSubst.of_constantCoeff_zero'`. -/
...
exactly. Provided as a convenience for T20 (a self-contained algebraic handle on
...
Paper: the conclusion `\tilde c_2(p) ∈ ℤ_p^×` of `conj:strong`. -/
...
lies in the maximal ideal `(p)`). Paper: `prop:consequence`, Step 2.
...
/-- The **normalised second jet** `\tilde c_2(p)` of `def:c2tilde`:
