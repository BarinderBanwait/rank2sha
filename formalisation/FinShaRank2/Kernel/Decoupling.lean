...
/-!
# Decoupling of the second jet (Lemma 6.3, `Kernel/Decoupling.lean`)

Pure power-series algebra behind Lemma 6.3 of *Second derivatives of
...
are applied to the comparison field of `KatzData`, which has the shape
...
* `coeff_two_mul_of_snd_low` / `coeff_two_mul_of_fst_low` — its collapse when the
  low coefficients of the second / first factor vanish (Lemma 6.3
  proper). The unit `u` of the comparison sits on the **left**, `LKatz` on the
  right, so `_of_snd_low` (vanishing on the right factor) is the variant that
  applies.
...
* `isUnit_coeff_two_of_comparison`: the fully composed forward transfer. From a
  comparison `f.map ι = c • (↑u · G)` with `c₀G = c₁G = 0` and `IsUnit (c₂G)`,
  conclude `IsUnit (c₂ f)`.

Paper statements quoted below: Lemma 6.3, Lemma 3.5.
-/
...
This is the ring-generic core of Lemma 6.3; the antidiagonal of `2` is
`{(0,2), (1,1), (2,0)}`. PAPER: Lemma 6.3. -/
...
`coeff 2 (A * B) = coeff 2 A * coeff 0 B`. PAPER: Lemma 6.3. -/
...
It turns the constant `c` of the comparison
...
Combined with `coeff_smul_eq_mul` and the constant-coefficient
...
This is the unit-transfer corollary of Lemma 6.3. It is applied with
`R := ℤ_[p]`, `S := W`, `ι := algebraMap ℤ_[p] W`, taking the
...
/-- **Fully composed grade-two transfer.** Given a
...
The derivation of the hypotheses `coeff 0 G = coeff 1 G = 0` is left to the
caller: it is the constant-coefficient bookkeeping on the unit series `u`,
combining `coeff_eq_zero_of_map_eq_zero`, `coeff_smul_eq_mul`, and `map_mul` on
`constantCoeff`. PAPER: Lemma 6.3, Lemma 3.5. -/
