...
This file renders the paper's Lemma 6.3 (`lem:orbit`, `invariants.tex`) as pure `mathlib`-only
...
* The paper applies Lemma 6.3 to `D_E` and `F_c` granting an equivariance input it does not
  prove: the reciprocity of Eisenstein–Kronecker values, `r_{a,b}(σ t) = σ (r_{a,b}(t))`
  (`invariants.tex`, the paragraph after Lemma 6.3). The algebraic trivialisation behind
...
* The paper records explicitly that Theorem C (Theorem 7.10) does not depend on this lemma
...
`[FiniteDimensional K L]`, which Lemma 6.3 does not assume.

Paper statements: Lemma 6.3, Definition 6.2, (9), Conjecture 7.9, Theorem C.
-/
...
(`InfiniteGalois.mem_range_algebraMap_iff_fixed`). PAPER: Lemma 6.3(1). -/
...
Neither finiteness of `D` nor the Galois hypothesis on `L/K` is used. PAPER: Lemma 6.3(2). -/
