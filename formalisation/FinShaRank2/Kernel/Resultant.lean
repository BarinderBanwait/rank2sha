...
valuation step in the proof of the paper's Theorem C (Theorem 7.10). Nothing here
...
Definition 6.2 sets `δ_E(c) = ∏_{t ∈ D_E} F_c(t)`. The proof of Theorem C
argues: each
factor `F_c(t)` is `𝔭`-integral by Hypothesis 7.8(i), so `v_𝔭(δ_E(c)) = ∑_{t ∈ D_E} v_𝔭(F_c(t))`
...
Paper statements: Theorem C, Definition 6.2, Hypothesis 7.8(i), §2.1.
-/
...
PAPER: Theorem C (Theorem 7.10, `thm:reduction`), proof; Hypothesis 7.8 (`hyp:sinnott`)(i). -/
...
Theorem C (Theorem 7.10) assumes `δ_E(c) ≠ 0` separately from `𝔭 ∤ δ_E(c)`; at the point where the
