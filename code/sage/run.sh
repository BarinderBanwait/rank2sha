...
# sides of eq:padicbsd for E : y^2 = x^3 - 56x, computed at a split ordinary p
# by disjoint methods and compared digit for digit.  The paper runs it at
# p = 5 and p = 13.
#
...
mkdir -p "$DATA"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/cert_p${P}_XXXXXX")"
...

{
  ...
  echo "generated by    : code/sage/run.sh $P $N $PRECT $CAP"
...
# Step 4 reads the file written so far and appends its verdict to it.
{
  ...
  python3 "$WORK/check_agreement.py" "$P" "$TMPOUT" "$LMFDB" 2>&1
  RC=$?
  echo "exit status     : $RC"
  echo
  echo "=== END p = $P ==="
