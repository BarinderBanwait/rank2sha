...
#
# The SECOND of the two independent implementations of Reg_p required by A5
# (the first is regulator.gp, using PARI/GP's ellpadicregulator).  Reproduces
# main.tex app:anchor: "The same regulators were recomputed independently in
# Sage (E.padic_regulator(p, 12)), agreeing digit for digit."
#
...
#     the main.tex Mordell-Weil basis P1 = (-7,7), P2 = (9,15)   [saturation check]
#   * the unit root alpha, the Euler factor (1-alpha^-1)^2, log_p(1+p)
#   * the height side (1-alpha^-1)^2 * Reg_p / log_p(1+p)^2 with Sha-factor 1.
...
print("anomalous?        : %s   (False = non-anomalous)" % ((p + 1 - ap) % p == 0))
...
print("v_p(Reg_p)        : %s   (expected 2)" % reg.valuation())
print("padic_regulator   : %.3f s" % (t1 - t0))

# --- Reg_p as a Gram determinant on the main.tex MW basis ---------------
...
# Machine-readable digit expansions.  Format shared with regulator.gp and
