...
every agreement claimed in docs/A5_REPORT.md is machine-verified inside the
committed output rather than asserted by a human reading two columns.

It parses the MACHINE lines emitted by regulator.gp, regulator.sage and
certificates.sage:
...
meaning the value is sum_i d_i * p^(v+i) + O(p^prec), and checks:
  ...
  C1  Reg_p: PARI ellpadicregulator vs Sage padic_regulator vs the copy used
      ...
      (This is the "two independent implementations agreeing digit for digit"
      of main.tex ssec:dictionary.)
  C2  Reg_p vs the explicit 2x2 Gram determinant of E.padic_height on the
      main.tex Mordell-Weil basis P1 = (-7,7), P2 = (9,15). Saturation check.
  C3  height side (1-alpha^-1)^2 Reg_p / log_p(1+p)^2: PARI vs Sage.
  C4  a_p: PARI ellap vs Sage E.ap.
  C5  eq:match5 -- height side vs L-side (modular symbols): equal valuation and
      equal digits over the full precision of the L-side.
  C6  the certificate itself: v_p(c_2(p)) = 0, hence (with lem:c0c1)
      lambda_an = 2 and mu_an = 0.
  C7  cross-check against Appendix B (../data/lmfdb_iwasawa.txt). LMFDB is
      corroboration only; a MISMATCH here is a stop-the-line event.
...
cmp_pair("reg.sage", "gram.sage", "C2  Reg_p  vs Gram det on (-7,7),(9,15)")
...
# C5 -- eq:match5
label = "C5  eq:match5  height side vs L-side"
...
# C7 -- Appendix B
label = "C7  vs Appendix B (LMFDB, corroboration)"
if lmfdb_path:
    lam_l = mu_l = None
    ...
    if lam_l is None:
        ...
        if ok:
            ...
            print("%-46s : *** MISMATCH *** LMFDB lambda=%s mu=%s, in-house lambda=%s mu=%s"
                  % (label, lam_l, mu_l, lam, mu))
            print("*** STOP THE LINE: in-house recomputation disagrees with Appendix B at p = %s" % p)
else:
