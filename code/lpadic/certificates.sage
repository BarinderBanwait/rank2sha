#!/usr/bin/env sage
# certificates.sage -- the A5 certificate at a split ordinary prime p for
#                      E : y^2 = x^3 - 56x   (conductor 12544 = 2^8 * 7^2).
#
# WHAT IT CERTIFIES
#   The L-side of eq:padicbsd, by modular symbols at level 12544 with the
#   provable truncation bounds of Stein-Wuthrich, exactly as at the two
#   existing anchors p = 5, 13 (main.tex sec:anchor, app:anchor):
#
#       L_p(E,T) = c_0 + c_1 T + c_2 T^2 + ...
#
#   computed by E.padic_lseries(p).series(n).  By lem:c0c1 (main.tex:884),
#   c_0 = c_1 = 0 exactly for every good ordinary p, so
#
#       v_p(c_2(p)) = 0   <==>   mu_an(p) = 0 and lambda_an(p) = 2,
#
#   which is hypothesis (v) of cor:sha5.  The script reports v_p(c_2(p)), the
#   base-p digits of c_2(p), and -- this is the point -- THE TRUNCATION BOUND
#   ACTUALLY ACHIEVED, so that "the leading digit is rigorous" is a checkable
...
#
# It also recomputes the HEIGHT side (1-alpha^-1)^2 * Reg_p / log_p(1+p)^2 with
# Sha-factor 1 and prints the eq:match5 digit-for-digit comparison.  The two
# independent implementations of Reg_p itself live in regulator.sage (Sage) and
# regulator.gp (PARI/GP).
#
...
print("### A5 certificate, L-side by modular symbols at level 12544")
...
print("anomalous         : %s   (lem:noanomalous: never, for this curve)"
      % ((p + 1 - ap) % p == 0))
print("rhobar irreducible: %s   (lem: c_bound = 0 below)"
...
# ---------------------------------------------------------------------------
# Height side of eq:padicbsd, and the eq:match5 comparison.
# ---------------------------------------------------------------------------
...
print("### eq:match5 comparison at p = %s" % p)
if digitsc2 is None:
